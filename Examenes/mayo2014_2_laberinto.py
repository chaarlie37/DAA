...
def laberinto(n, tablero, pasos_minimos, camino_parcial, p, pasos_acumulados, mejor_sol):
    meta = [n - 1, n - 1]
    if p == meta:
        if len(camino_parcial) > pasos_acumulados:
            camino_parcial[pasos_acumulados] = p
        else:
        ...
        if pasos_acumulados <= mejor_sol[0]:
            ...
            for k in camino_parcial:
                if tablero[k[0]][k[1]] == "*":
                    tablero[k[0]][k[1]] = " "
            for k in camino_parcial[:pasos_acumulados + 1]:
                tablero[k[0]][k[1]] = "*"
        ...
        if pasos_acumulados < mejor_sol[0] and pasos_acumulados <= pasos_minimos[p[0]][p[1]]:
            ...
            for i in range(4):
                ...
                if nuevo_p != p and (nuevo_p not in camino_parcial) and (
                    ...
                    laberinto(n, tablero, pasos_minimos, camino_parcial[:pasos_acumulados + 1], nuevo_p,
                              pasos_acumulados + 1,
                              mejor_sol)
...
laberinto(10, tablero, m, [], [0, 0], 0, [60])
imprimir_matriz(m)
